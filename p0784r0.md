...
* the storage of object of type `T` that has been destroyed and not yet (in part or in whole)
  reconstructed, or
...
For example:
    ...
    #ifdef QUESTION
      new(p) double{0.0};
        // Does constexpr evaluation fail because
...
that the the container templates in the [containers] clause be usable in
